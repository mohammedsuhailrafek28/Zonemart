...
export function Header() {
  ...
  return <header className="site-header"><nav className="container nav" aria-label="Main navigation">
    ...
    <div className="nav-links">
      ...
      {!loading && !session && <><Link href="/#how-it-works">How it works</Link><Link href="/flash-requests/new">Flash Requests</Link><Link href="/#for-merchants">For merchants</Link><Link href="/auth/sign-in">Sign in</Link><Link className="button small" href="/shop">Search nearby</Link></>}
    ...
    <div className="mobile-nav">
      ...
      {!loading && !session && <><Link href="/auth/sign-in">Sign in</Link><Link className="button small" href="/shop">Search</Link></>}
      {session && <button className="button secondary small" onClick={() => void signOut()}>Sign out</button>}
    </div>
  </nav></header>;
}
